...
func (e *Engine) ExportRuntimeState() (RuntimeState, error) {
	...
	state := RuntimeState{
		Version: RuntimeStateVersion, MIBDataSync: e.mib.DataSync(),
...
func (e *Engine) RestoreRuntimeState(state RuntimeState) error {
	...
	gem, fec, ethernet, histories, err := e.validateRuntimePerformanceLocked(state, instances)
	if err != nil {
		return err
	}
...
func (e *Engine) validateRuntimePerformanceLocked(state RuntimeState,
	...
	map[mib.Key]me.AttributeValueMap, error) {
	...
	histories := make(map[mib.Key]me.AttributeValueMap,
		len(state.GEMPerformance)+len(state.FECPerformance)+len(state.EthernetPerformance))

	for _, value := range state.GEMPerformance {
		if value.Key.ClassID != me.GemPortNetworkCtpPerformanceMonitoringHistoryDataClassID {
			return nil, nil, nil, nil, fmt.Errorf("invalid GEM runtime key %v/%#x",
				value.Key.ClassID, value.Key.EntityID)
		}
		if _, duplicate := gem[value.Key]; duplicate {
			return nil, nil, nil, nil, fmt.Errorf("duplicate GEM runtime key %#x", value.Key.EntityID)
		}
		if e.performance == nil {
			return nil, nil, nil, nil, fmt.Errorf("GEM runtime state has no counter backend")
		}
		...
		if err != nil || parent.Attributes[me.GemPortNetworkCtp_PortId] != value.PortID {
			return nil, nil, nil, nil, fmt.Errorf("GEM runtime key %#x has a stale port ID",
				value.Key.EntityID)
		}
		current, err := e.performance.GEMPortCounters(value.PortID)
		if err != nil {
			return nil, nil, nil, nil, fmt.Errorf("validate GEM port %d counters: %w", value.PortID, err)
		}
		if !gemCountersAtLeast(current, value.Baseline) {
			return nil, nil, nil, nil, fmt.Errorf("GEM port %d counters reset below the saved baseline", value.PortID)
		}
		history, err := restoreRuntimeHistory(instances[value.Key], value.History)
		if err != nil {
			return nil, nil, nil, nil, fmt.Errorf("restore GEM history %#x: %w", value.Key.EntityID, err)
		}
	...
	for _, value := range state.FECPerformance {
		if value.Key.ClassID != me.FecPerformanceMonitoringHistoryDataClassID ||
			...
			return nil, nil, nil, nil, fmt.Errorf("invalid FEC runtime key %v/%#x",
				value.Key.ClassID, value.Key.EntityID)
		}
		if _, duplicate := fec[value.Key]; duplicate {
			return nil, nil, nil, nil, fmt.Errorf("duplicate FEC runtime key %#x", value.Key.EntityID)
		}
		if e.fecPerformance == nil || !e.mib.Exists(mib.Key{ClassID: me.AniGClassID,
			EntityID: value.ANIEntityID}) {
			return nil, nil, nil, nil, fmt.Errorf("FEC runtime key %#x has no ANI counter backend", value.Key.EntityID)
		}
		current, err := e.fecPerformance.FECCounters(value.ANIEntityID)
		if err != nil {
			return nil, nil, nil, nil, fmt.Errorf("validate ANI-G %#x FEC counters: %w", value.ANIEntityID, err)
		}
		if !fecCountersAtLeast(current, value.Baseline) {
			return nil, nil, nil, nil, fmt.Errorf("ANI-G %#x FEC counters reset below the saved baseline", value.ANIEntityID)
		}
		history, err := restoreRuntimeHistory(instances[value.Key], value.History)
		if err != nil {
			return nil, nil, nil, nil, fmt.Errorf("restore FEC history %#x: %w", value.Key.EntityID, err)
		}
	...
	for _, value := range state.EthernetPerformance {
		if !isEthernetPerformanceClass(value.Key.ClassID) {
			return nil, nil, nil, nil, fmt.Errorf("invalid Ethernet runtime key %v/%#x",
				value.Key.ClassID, value.Key.EntityID)
		}
		if _, duplicate := ethernet[value.Key]; duplicate {
			return nil, nil, nil, nil, fmt.Errorf("duplicate Ethernet runtime key %v/%#x",
				value.Key.ClassID, value.Key.EntityID)
		}
		if e.ethernetPerformance == nil {
			return nil, nil, nil, nil, fmt.Errorf("Ethernet runtime state has no counter backend")
		}
		resolved, err := e.resolveEthernetPerformanceUNILocked(value.Key)
		if err != nil || resolved != value.UNIEntityID {
			return nil, nil, nil, nil, fmt.Errorf("Ethernet runtime key %v/%#x has a stale UNI",
				value.Key.ClassID, value.Key.EntityID)
		}
		current, err := e.ethernetPerformance.EthernetCounters(value.UNIEntityID)
		if err != nil {
			return nil, nil, nil, nil, fmt.Errorf("validate Ethernet UNI %#x counters: %w", value.UNIEntityID, err)
		}
		if !ethernetCountersAtLeast(current, value.Baseline) {
			return nil, nil, nil, nil, fmt.Errorf("Ethernet UNI %#x counters reset below the saved baseline", value.UNIEntityID)
		}
		history, err := restoreRuntimeHistory(instances[value.Key], value.History)
		if err != nil {
			return nil, nil, nil, nil, fmt.Errorf("restore Ethernet history %v/%#x: %w",
				value.Key.ClassID, value.Key.EntityID, err)
		}
	...
	for key, instance := range instances {
		...
		case key.ClassID == me.GemPortNetworkCtpPerformanceMonitoringHistoryDataClassID && e.performance != nil:
			if _, present := gem[key]; !present {
				return nil, nil, nil, nil, fmt.Errorf("runtime state is missing GEM performance %#x", key.EntityID)
			}
		case key.ClassID == me.FecPerformanceMonitoringHistoryDataClassID && e.fecPerformance != nil:
			if _, present := fec[key]; !present {
				return nil, nil, nil, nil, fmt.Errorf("runtime state is missing FEC performance %#x", key.EntityID)
			}
		case isEthernetPerformanceClass(instance.ClassID) && e.ethernetPerformance != nil:
			if _, present := ethernet[key]; !present {
				return nil, nil, nil, nil, fmt.Errorf("runtime state is missing Ethernet performance %v/%#x",
					key.ClassID, key.EntityID)
			}
		}
	}
	return gem, fec, ethernet, histories, nil
}
